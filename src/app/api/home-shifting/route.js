...
async function ensureHomeShiftingQuoteSchema() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS home_shifting_quotes (
    ...
    ALTER TABLE IF EXISTS home_shifting_quotes
...
export async function GET() {
  try {
    ...
    console.error("[home-shifting] GET error:", err.message, "| code:", err.code);
...
export async function POST(req) {
  try {
    ...
    await ensureHomeShiftingQuoteSchema();
    ...
    try {
      ...
      const message = txErr.code === "ECONNREFUSED"
        ? "Database connection failed. Please make sure PostgreSQL is running."
        : txErr.message || "Something went wrong. Please try again.";
      return NextResponse.json(
        { error: message, code: txErr.code, constraint: txErr.constraint },
        { status: 500 }
      );
    } finally {
      client.release();
