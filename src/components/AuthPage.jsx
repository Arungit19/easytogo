...
export default function AuthPage() {
  ...
  const inputCls =
    "w-full min-w-0 border border-blue-200 rounded-lg px-3 py-2.5 sm:py-2 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500";
  const primaryBtn =
    "w-full py-2.5 rounded-lg font-semibold uppercase tracking-wide text-white bg-blue-600 hover:bg-blue-700 active:scale-[0.98] transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed text-sm";
  const secondaryBtn =
    "w-full py-2.5 rounded-lg font-semibold text-blue-600 border border-blue-300 hover:bg-blue-50 active:scale-[0.98] transition-all duration-200 text-sm mt-2";
  const toggleActive =
    "flex-1 min-w-0 px-2 py-2 sm:py-1.5 text-xs sm:text-sm font-medium bg-blue-600 text-white transition-colors";
  const toggleInactive =
    "flex-1 min-w-0 px-2 py-2 sm:py-1.5 text-xs sm:text-sm font-medium bg-white text-blue-600 border-blue-200 hover:bg-blue-50 transition-colors";
  ...
  const handleKeyDown = (e, idx, type = "login") => {
    if (e.key === "Backspace") {
      if (type === "login" && !otp[idx] && idx > 0)
        document.getElementById(`otp-${idx - 1}`)?.focus();
      if (type === "signup" && !signupOtp[idx] && idx > 0)
        document.getElementById(`sotp-${idx - 1}`)?.focus();
      if (type === "forgot" && !forgotOtp[idx] && idx > 0)
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const body = isValidEmail(identifier)
        ...
        : { phone: identifier.trim(), otp: code };
  ...
  const handleResendOTP = async (emailOrPhone) => {
    ...
    try {
      const body = isValidEmail(emailOrPhone)
        ? { email: emailOrPhone }
        : { phone: emailOrPhone };
      const res = await fetch(`${API}/auth/send-otp`, {
        method: "POST",
  ...
  const handleSignupSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        setSuccessMsg(`Account created! An OTP has been sent to ${email} for verification (optional).`);
      } catch {
  ...
  const handleSkipOtp = async () => {
    if (pendingToken && pendingUser) {
      await saveAndRedirect(pendingToken, pendingUser);
    } else {
      setLoading(true);
      try {
        const res = await fetch(`${API}/auth/login`, {
        ...
        const data = await res.json();
        if (!res.ok) { setError(data.message || "Auto-login failed. Please log in manually."); return; }
        await saveAndRedirect(data.data.token, data.data.user);
      } catch { setError("Could not auto-login. Please log in manually."); }
      finally { setLoading(false); }
    }
  };
    ...
    <div className="grid grid-cols-6 gap-1.5 sm:gap-2 mb-4">
      {values.map((digit, i) => (
        <input
          ...
          value={digit}
          onChange={(e) =>
            handleOtpChange(e.target.value.replace(/[^0-9]/g, ""), i, type)
          }
          onKeyDown={(e) => handleKeyDown(e, i, type)}
          onPaste={(e) => handleOtpPaste(e, type)}
          className="h-10 w-full min-w-0 rounded-lg border border-blue-300 text-center text-base font-semibold text-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 sm:h-11 sm:text-lg"
        />
      ))}
    </div>
  );
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
        <button
          ...
          className="flex min-w-0 items-center justify-center gap-2 rounded-lg border border-gray-200 px-4 py-2.5 transition-all duration-200 hover:bg-gray-50 hover:shadow-md sm:py-2"
        >
  ...
  // ── Worker Login Button — reusable ───────────────────────────────────────
  const WorkerLoginButton = () => (
    <button
      onClick={() => router.push("/worker/login")}
      className="w-full mt-3 flex items-center justify-center gap-2 py-2.5 rounded-lg border border-[rgba(192,57,43,0.35)] bg-[rgba(192,57,43,0.08)] text-[#c0392b] hover:bg-[#c0392b] hover:border-[#c0392b] hover:text-white active:scale-[0.98] transition-all duration-200 text-sm font-semibold shadow-[0_4px_18px_rgba(192,57,43,0.16)] group"
    >
      <HardHat
        size={15}
        className="text-[#c0392b] group-hover:text-white transition-colors"
      />
  ...
  return (
    <>
      ...
      <main className="flex min-h-[100svh] items-start justify-center overflow-y-auto px-3 py-4 sm:px-4 sm:py-6 md:items-center lg:py-8">
        <div className="relative flex w-full max-w-md overflow-hidden rounded-2xl shadow-2xl md:max-w-3xl lg:max-w-4xl">

          {/* LEFT PROMO PANEL */}
          <div className="relative hidden w-2/5 flex-col items-center justify-center bg-gradient-to-b from-blue-800 to-blue-500 p-6 text-white md:flex lg:p-8">
            <button
              onClick={() => router.push("/")}
            ...
            {/* Worker link on left panel
            <button
              onClick={() => router.push("/worker/login")}
              className="absolute bottom-4 flex items-center gap-1.5 text-white/50 hover:text-white/80 text-xs transition-colors"
            >
              <HardHat size={12} />
          ...
          {/* RIGHT FORM PANEL */}
          <div className="relative flex w-full flex-col bg-white/95 backdrop-blur-md md:w-3/5">
            <Image
              src="/logo.png"
              alt="logo"
              width={90}
              height={72}
              className="absolute right-3 top-3 z-10 h-auto w-16 sm:w-[90px] md:hidden"
            />
            ...
            <div className="flex min-h-28 items-end bg-gradient-to-r from-blue-800 to-blue-500 p-3 pt-16 text-white sm:min-h-32 md:hidden">
              <div>
            ...
            <div className="w-full overflow-y-auto p-4 sm:p-5 md:max-h-[calc(100svh-3rem)] lg:p-6">
                ...
                <>
                  ...
                  {loginMethod === "password" && (
                    <>
                      ...
                      <div className="mb-3 flex flex-col gap-2 min-[360px]:flex-row min-[360px]:items-center min-[360px]:justify-between">
                        <div className="flex items-center gap-1.5">
                          ...
                          />
                          <label htmlFor="showPwd" className="text-xs text-gray-600 cursor-pointer">
                            Show Password
                          </label>
                        </div>
                        <button
                  ...
                  <div className="text-center mt-3 text-xs text-gray-500">
                    ...
                    <button
                      onClick={() => { setStep("signup"); setError(""); setSuccessMsg(""); }}
              ...
              {step === "otp" && (
                <>
                  <button
                    onClick={() => { setStep("login"); setOtp(["", "", "", "", "", ""]); setError(""); setSuccessMsg(""); }}
                    className="text-blue-600 mb-2 text-xs hover:underline flex items-center gap-1 font-medium"
                  >
                    &lt;- Back to Login
                  </button>
                  <h2 className="text-lg font-bold text-blue-800 mb-0.5">Enter OTP</h2>
                  <p className="text-xs text-gray-400 mb-1">
                    Code sent to <span className="font-medium text-blue-700">{identifier}</span>
                  </p>
                  ...
                  <p className="text-center text-xs text-gray-400 mt-3">
                    Did not receive OTP?{" "}
                    <button
                      onClick={() => handleResendOTP(identifier)}
                      className="text-blue-600 hover:underline font-medium"
                    >
                      Resend OTP
                    </button>
                ...
                <>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="flex gap-2">
                      <span className="flex shrink-0 items-center rounded-lg border border-blue-200 bg-blue-50 px-2.5 text-xs font-medium text-blue-700">
                        +91
                      </span>
                      <input
                        type="tel"
                        ...
                        onChange={(e) => {
                          ...
                          if (error === "__PHONE_EXISTS__") setError("");
                        }}
                        placeholder="10-digit mobile number"
                        className={`${inputCls} flex-1 min-w-0 ${
                          error === "__PHONE_EXISTS__"
                      ...
                      <div className="mt-2 flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-red-500 mt-0.5">
                            Please use a different number, or{" "}
                            <button
                              type="button"
                              onClick={() => {
                                setStep("login");
                  ...
                  <div className="mb-2">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <input
                        ...
                        className="w-3.5 h-3.5 cursor-pointer accent-blue-600"
                      />
                      <label htmlFor="showSPwd" className="text-xs text-gray-500 cursor-pointer">
                        Show Password
                      </label>
                    </div>
                  </div>
                  <div className="mb-3">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <input
                        ...
                        className="w-3.5 h-3.5 cursor-pointer accent-blue-600"
                      />
                      <label htmlFor="showCPwd" className="text-xs text-gray-500 cursor-pointer">
                        Show Password
                      </label>
                    </div>
                  </div>

                  {error && error !== "__PHONE_EXISTS__" && (
                ...
                <>
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 mb-3">
                    ...
                    <p className="text-xs text-green-600 mt-0.5">
                      You can enter the OTP sent to{" "}
                      <span className="font-semibold">{signupData.email}</span> to verify,
                      or skip and start using the app right away.
                    </p>
                  </div>

                  <p className="text-xs text-gray-400 mb-1">
                    Enter the 6-digit code (optional):
                  </p>
                  <p className="text-xs text-gray-400 mb-4">
                    Tip: Copy the OTP and paste it - all boxes fill instantly.
                  </p>
                  ...
                  <button onClick={handleSkipOtp} disabled={loading} className={secondaryBtn}>
                    {loading ? "Please wait..." : "Skip Verification & Continue ->"}
                  </button>
              ...
              {step === "forgotOtp" && (
                <>
                  ...
                  <p className="text-xs text-gray-400 mb-1">
                    ...
                    <span className="font-medium text-blue-700">{forgotInput}</span>
                  </p>
                  <p className="text-xs text-gray-400 mb-4">
                    Tip: Copy the OTP and paste it - all boxes fill instantly.
                  </p>
                  ...
                  <p className="text-center text-xs text-gray-400 mt-3">
                    Did not receive OTP?{" "}
                    <button
                      onClick={() => handleResendOTP(forgotInput)}
                      className="text-blue-600 hover:underline font-medium"
                    >
                      Resend OTP
                    </button>
